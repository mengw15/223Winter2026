...
public class ExperimentRunner {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Hotset size: " + HOTSET_SIZE);
        ...
        for (TransactionManager.Protocol proto : TransactionManager.Protocol.values()) {
            for (int threads : THREAD_COUNTS) {
                for (double contention : CONTENTION_LEVELS) {
                    ...
                    tm.runWorkload(keyPools, HOTSET_SIZE, contention, threads, NUM_TRANSACTIONS, templates);

                    // Export CSV
                    Stats.appendSummary(1, proto.toString(), threads, contention, HOTSET_SIZE, NUM_TRANSACTIONS,
        ...
        for (TransactionManager.Protocol proto : TransactionManager.Protocol.values()) {
            for (int threads : THREAD_COUNTS) {
                for (double contention : CONTENTION_LEVELS) {
                    ...
                    Stats.appendSummary(2, proto.toString(), threads, contention, HOTSET_SIZE, NUM_TRANSACTIONS,
                            tm.getTotalCommitted(), tm.getTotalRetries(), tm.getLastRetryRate(),
                            tm.getLastThroughput(), tm.getAvgResponseTimeMs());
